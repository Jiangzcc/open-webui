...
- **Endpoint**: `https://fal.run/google/nano-banana-lite`
...
- **`images`** (`list<ImageFile>`, _required_):
  ...
  - Array of ImageFile
  - Examples: [{"url":"https://storage.googleapis.com/falserverless/example_outputs/nano-banana-2-t2i-output.png","file_name":"nano-banana-lite-t2i-output.png","content_type":"image/png"}]
...
```bash
curl --request POST \
  --url https://fal.run/google/nano-banana-lite \
...
const result = await fal.subscribe("google/nano-banana-lite", {
  input: {
    prompt: "An action shot of a black lab swimming in an inground suburban swimming pool. The camera is placed meticulously on the water line, dividing the image in half, revealing both the dogs head above water holding a tennis ball in it's mouth, and it's paws paddling underwater."
  },
  logs: true,
...
- [Model Playground](https://fal.ai/models/google/nano-banana-lite)
- [API Documentation](https://fal.ai/models/google/nano-banana-lite/api)
- [OpenAPI Schema](https://fal.ai/api/openapi/queue/openapi.json?endpoint_id=google/nano-banana-lite)
