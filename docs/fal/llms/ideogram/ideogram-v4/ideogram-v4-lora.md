# Ideogram V4.0q Text to Image (LoRA)
...
- **Endpoint**: `https://fal.run/ideogram/v4/lora`
- **Model ID**: `ideogram/v4/lora`
...
Your request will cost **$0.01125** per megapixel in **TURBO** mode, **$0.0225** per megapixel in **BALANCED** mode, or **$0.0375** per megapixel in **QUALITY** mode. For example, a 2048 x 2048 image will cost **$0.045**, **$0.09** or **$0.15** based on the mode.
...
```bash
curl --request POST \
  --url https://fal.run/ideogram/v4/lora \
...
const result = await fal.subscribe("ideogram/v4/lora", {
  input: {
    prompt: "A red panda perched on a mossy branch in a misty forest at sunrise"
  },
  logs: true,
...
- [Model Playground](https://fal.ai/models/ideogram/v4/lora)
- [API Documentation](https://fal.ai/models/ideogram/v4/lora/api)
- [OpenAPI Schema](https://fal.ai/api/openapi/queue/openapi.json?endpoint_id=ideogram/v4/lora)
