...
_LOCK_NAMESPACE = 'open-webui-provider-ops-migrations'


def _migration_config(schema: str | None) -> Config:
    config = Config()
    config.set_main_option('script_location', str(Path(__file__).parent))
    config.attributes['target_metadata'] = ProviderOpsBase.metadata
    config.attributes['provider_ops_schema'] = DATABASE_SCHEMA if schema is None else schema
    return config
...
def _sqlite_lock_path(connection: Connection) -> Path | None:
    ...
    return path.with_name(f'{path.name}.provider-ops-migrations.lock')
...
def run_provider_ops_migrations(  # noqa: C901 - lock acquisition and cleanup share one transaction boundary
    ...
    *,
    ...
    if live.in_transaction():
        if owned:
            live.close()
        raise RuntimeError('provider ops migrations require a connection without an active transaction')
...
__all__ = ['run_provider_ops_migrations']
