...
_LOCK_NAMESPACE = 'open-webui-credits-migrations'
...
def _sqlite_lock_path(connection: Connection) -> Path | None:
    ...
    return path.with_name(f'{path.name}.credit-migrations.lock')
...
def _acquire_postgres_lock(connection: Connection) -> int:
    ...
    while True:
        ...
        if acquired:
            return key
        if time.monotonic() >= deadline:
            raise TimeoutError('timed out acquiring PostgreSQL credit migration lock')
        time.sleep(_LOCK_POLL_SECONDS)
...
def _migration_config(schema: str | None) -> Config:
    ...
    config.attributes['target_metadata'] = CreditBase.metadata
    config.attributes['credit_schema'] = DATABASE_SCHEMA if schema is None else schema
    return config
    ...
    try:
        ...
        try:
            ...
            if migration_error is None:
                raise
            log.exception('Failed to release PostgreSQL credit migration lock after migration failure')
...
def _run_non_postgres_migration(connection: Connection, verify_upstream: bool, schema: str | None) -> None:
    ...
    try:
        ...
        try:
            ...
            if migration_error is None:
                raise
            log.exception('Failed to release credit migration lock after migration failure')
...
def run_credit_migrations(
    ...
    *,
    ...
    if live_connection.in_transaction():
        ...
        raise RuntimeError('credit migrations require a connection without an active transaction')
