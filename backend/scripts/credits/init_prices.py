...
用法：
  cd backend
  python scripts/credits/init_prices.py
"""
...
_CATALOG_DIR = (
    Path(__file__).resolve().parents[2] / 'open_webui' / 'extensions' / 'fal_catalog' / 'catalog'
)
...
_VIDEO_TIER_BASE: dict[str, tuple[float, str]] = {
    ...
    # seedance-2.0 fast: 720p $0.2419/s（API 返回的 $0.0112 是每千 token 价而非每秒价，
    # 720p 每秒 21600 token，直接当每秒价会少收 21.6 倍）
    'bytedance/seedance-2.0/fast/text-to-video': (0.2419, 'per_second'),
    'bytedance/seedance-2.0/fast/image-to-video': (0.2419, 'per_second'),
...
_VIDEO_RESOLUTION_TIERS: dict[str, dict[str, str]] = {
    ...
    # seedance-2.0: 720p $0.3034/s 为基准，1080p $0.682/s → 2.248。
    # 480p/4k 无 per-second 价，FAL 按 token 计费（tokens = w×h×duration×24/1024，
    # 480p/720p/1080p $0.014/千 token、4k $0.008/千 token），换算等价每秒价：
    # 480p(854×480) $0.1345/s → 0.443；4k(3840×2160) $1.5552/s → 5.126。
    'bytedance/seedance-2.0/text-to-video': {'480p': '0.443', '720p': '1', '1080p': '2.248', '4k': '5.126', 'default': '1'},
    'bytedance/seedance-2.0/image-to-video': {'480p': '0.443', '720p': '1', '1080p': '2.248', '4k': '5.126', 'default': '1'},
    # seedance-2.0 fast: 720p $0.2419/s 为基准；fast token 价 $0.0112/千 token，
    # 480p(854×480) $0.1076/s → 0.445。fast 仅支持 480p/720p。
    'bytedance/seedance-2.0/fast/text-to-video': {'480p': '0.445', '720p': '1', 'default': '1'},
    'bytedance/seedance-2.0/fast/image-to-video': {'480p': '0.445', '720p': '1', 'default': '1'},
...
def generate_init_script(records: list[dict], output_path: Path) -> None:
    ...
    lines = [
        ...
        '用法：',
        '  cd backend',
        '  WEBUI_SECRET_KEY=<key> PYTHONPATH=. python -m scripts.credits.seed_prices',
        '"""',
        'from __future__ import annotations',
        '',
        'import json',
...
def generate_excel(records: list[dict], output_path: Path) -> None:
    ...
    notes = [
        ...
        ['seedance-2.0', 'units 实际为按秒计费（分层定价取文档 720p per-second 价；480p/4k 按 token 价换算等价每秒价；fast 的 API 价 $0.0112 是每千 token 价，不可当每秒价）'],
