# LTX Video 2.3 Pro

> LTX-2.3 is a high-quality, fast AI video model available in Pro and Fast variants for text-to-video, image-to-video, and audio-to-video.


## Overview

- **Endpoint**: `https://fal.run/fal-ai/ltx-2.3/retake-video`
- **Model ID**: `fal-ai/ltx-2.3/retake-video`
- **Category**: video-to-video
- **Kind**: inference
**Tags**: stylized, transform, lipsync



## Pricing

Your request will cost **$0.10 per second.**

For more details, see [fal.ai pricing](https://fal.ai/pricing).

## API Information

This model can be used via our HTTP API or more conveniently via our client libraries.
See the input and output schema below, as well as the usage examples.


### Input Schema

The API accepts the following input parameters:


- **`video_url`** (`string`, _required_):
  The URL of the video to retake
  - Examples: "https://v3b.fal.media/files/b/0a90e00e/eNWXGtaGyZO--e6KcJh-3_171_video.mp4"

- **`prompt`** (`string`, _required_):
  The prompt to retake the video with
  - Examples: "Change flower to red rose"

- **`start_time`** (`float`, _optional_):
  The start time of the video to retake in seconds
  - Default: `0`
  - Range: `0` to `20`

- **`duration`** (`float`, _optional_):
  The duration of the video to retake in seconds Default value: `5`
  - Default: `5`
  - Range: `2` to `20`

- **`retake_mode`** (`RetakeModeEnum`, _optional_):
  The retake mode to use for the retake Default value: `"replace_audio_and_video"`
  - Default: `"replace_audio_and_video"`
  - Options: `"replace_audio"`, `"replace_video"`, `"replace_audio_and_video"`



**Required Parameters Example**:

```json
{
  "video_url": "https://v3b.fal.media/files/b/0a90e00e/eNWXGtaGyZO--e6KcJh-3_171_video.mp4",
  "prompt": "Change flower to red rose"
}
```

**Full Example**:

```json
{
  "video_url": "https://v3b.fal.media/files/b/0a90e00e/eNWXGtaGyZO--e6KcJh-3_171_video.mp4",
  "prompt": "Change flower to red rose",
  "duration": 5,
  "retake_mode": "replace_audio_and_video"
}
```


### Output Schema

The API returns the following output format:

- **`video`** (`VideoFile`, _required_):
  The generated video file
  - Examples: {"file_name":"xvl9SlF6b7a8327wKJDvm_JW6crfr3.mp4","content_type":"video/mp4","url":"https://v3b.fal.media/files/b/0a90e00f/xvl9SlF6b7a8327wKJDvm_JW6crfr3.mp4"}



**Example Response**:

```json
{
  "video": {
    "file_name": "xvl9SlF6b7a8327wKJDvm_JW6crfr3.mp4",
    "content_type": "video/mp4",
    "url": "https://v3b.fal.media/files/b/0a90e00f/xvl9SlF6b7a8327wKJDvm_JW6crfr3.mp4"
  }
}
```


## Usage Examples

### cURL

```bash
curl --request POST \
  --url https://fal.run/fal-ai/ltx-2.3/retake-video \
  --header "Authorization: Key $FAL_KEY" \
  --header "Content-Type: application/json" \
  --data '{
     "video_url": "https://v3b.fal.media/files/b/0a90e00e/eNWXGtaGyZO--e6KcJh-3_171_video.mp4",
     "prompt": "Change flower to red rose"
   }'
```

### Python

Ensure you have the Python client installed:

```bash
pip install fal-client
```

Then use the API client to make requests:

```python
import fal_client

def on_queue_update(update):
    if isinstance(update, fal_client.InProgress):
        for log in update.logs:
           print(log["message"])

result = fal_client.subscribe(
    "fal-ai/ltx-2.3/retake-video",
    arguments={
        "video_url": "https://v3b.fal.media/files/b/0a90e00e/eNWXGtaGyZO--e6KcJh-3_171_video.mp4",
        "prompt": "Change flower to red rose"
    },
    with_logs=True,
    on_queue_update=on_queue_update,
)
print(result)
```

### JavaScript

Ensure you have the JavaScript client installed:

```bash
npm install --save @fal-ai/client
```

Then use the API client to make requests:

```javascript
import { fal } from "@fal-ai/client";

const result = await fal.subscribe("fal-ai/ltx-2.3/retake-video", {
  input: {
    video_url: "https://v3b.fal.media/files/b/0a90e00e/eNWXGtaGyZO--e6KcJh-3_171_video.mp4",
    prompt: "Change flower to red rose"
  },
  logs: true,
  onQueueUpdate: (update) => {
    if (update.status === "IN_PROGRESS") {
      update.logs.map((log) => log.message).forEach(console.log);
    }
  },
});
console.log(result.data);
console.log(result.requestId);
```


## Additional Resources

### Documentation

- [Model Playground](https://fal.ai/models/fal-ai/ltx-2.3/retake-video)
- [API Documentation](https://fal.ai/models/fal-ai/ltx-2.3/retake-video/api)
- [OpenAPI Schema](https://fal.ai/api/openapi/queue/openapi.json?endpoint_id=fal-ai/ltx-2.3/retake-video)

### fal.ai Platform

- [Platform Documentation](https://fal.ai/docs/documentation)
- [Python Client](https://fal.ai/docs/api-reference/client-libraries/python)
- [JavaScript Client](https://fal.ai/docs/api-reference/client-libraries/javascript)
